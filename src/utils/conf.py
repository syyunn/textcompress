...
def argparse_attr(default=attr.NOTHING,
                  validator=None,
                  repr=True,
                  cmp=True,
                  hash=True,
...
class EnvConfiguration:
    ...
    def from_env(cls):
        try:
            ...
            env_config_path = \
                "/home/zachary/projects/usc_dae/env_configs/env_config.json"
        except KeyError:
...
class Configuration:
    ...
    def __attrs_post_init__(self):
        if self.env_config_path:
            self.env = EnvConfiguration.from_json_path\
    ...
    def to_dict(self):
        ...
        for attribute in inspect.getfullargspec(Configuration)[0]:
            if attribute == "self":
                continue
            config_dict[attribute] = getattr(self,
                                             attribute)
        return config_dict
